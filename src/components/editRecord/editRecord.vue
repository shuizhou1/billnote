  ...
  <section class="account">
    ...
    <div class="accountBody">
      <div class="billNum clearfix">
        <span  class="billNumText">金额</span>
        <input type="text" id="billNum" name="billNum" v-model="billNum" v-focus placeholder="请输入金额">
        <!-- <span>{{billNum}}</span> -->
      </div>
      <div class="billInout clearfix">
        <span>收支</span>
      ...
      <button type="submit" class="btn" @click="editRecord" :style="{color: color, border: `.1rem solid ${color}` }">保存修改</button>
    </div>
  </section>
</template>
<script>
export default {
  data() {
    return {
      ...
      date: "",
      remarks: "", // 备注
      billNum: "", // 金额  将这几项保存在state里避免选择类别后丢失数据
      subdivide: "",
      icon: "",
      id: "",
      //   以上首页及分类流水进入时通过路由传递的参数
  ...
  methods: {
    ...
    initialData() {
      if (this.$route.params.obj) {
        ...
        this.radio = String(this.$route.params.obj.income_expenditure),
        ...
        this.categoryID = this.$route.params.obj.classification_id;
        this.categoryName = this.$route.params.obj.classification;
        this.subdivide_id = this.$route.params.obj.subdivide_id;
      } else {
        this.id = this.$store.state.id;
        this.radio = this.$store.state.radio;
        this.date = this.$store.state.date;
        this.remarks = this.$store.state.billRemarks;
        this.billNum = this.$store.state.billNum;
        this.icon = this.$store.state.billcategoryParams.icon;
        this.subdivide = this.$store.state.billcategoryParams.subdivide;
        this.subdivide_id = this.$store.state.billcategoryParams.subdivideID;
        this.categoryName = this.$store.state.billcategoryParams.superName;
        this.categoryID = this.$store.state.billcategoryParams.superID;
      }
    },
    // 跳转到分类选择页面
    toBillCategory() {
    ...
    editRecord() {
      this.$http
        .post("http://billing.gzpeifeng.com/user/bill/edit", {
          id: this.id,
    ...
    initialDate() {
      ...
      if (this.$route.params.superID) {
        // 从支出分类进入时显示选择的日期，从账本页进入时显示当天日期
        this.date = this.$store.state.date;
      } else {
